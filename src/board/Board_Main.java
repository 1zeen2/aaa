...
import javax.swing.*;
import java.util.*;
public class Board_Main extends JFrame implements ActionListener,MouseListener{
	// 인터페이스 => 다중 상속이 가능 
	// 1. button => ActionListener 
	// 2. table => MouseListener 
    private CardLayout card=new CardLayout();
    Board_List bList=new Board_List();
    Board_Insert bInsert=new Board_Insert();
    Board_Detail bDetail=new Board_Detail();
    Board_Update bUpdate=new Board_Update();
    // 클래스 => 관련있는 클래스별로 선언 
    // 프로그램에 필요한 변수 설정 
    int curpage=1;
    int totalpage=0;
    
    Board_System bs=new Board_System(); // 파일에 List 저장됨 
    public Board_Main()
    {
    	setLayout(card);
